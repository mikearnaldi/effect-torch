  ...
  describe("values", () => {
    it.effect("mse / l1 / huber", () =>
      Effect.gen(function* () {
        ...
        expect(Math.abs(mseValue - (0 + 4 + 9) / 3)).toBeLessThan(1e-12)
        const [l1Value] = yield* values(yield* Loss.l1(pred, target))
        expect(Math.abs(l1Value - (0 + 2 + 3) / 3)).toBeLessThan(1e-12)
        const [huberValue] = yield* values(yield* Loss.huber(pred, target))
        expect(Math.abs(huberValue - (0 + 1.5 + 2.5) / 3)).toBeLessThan(1e-12)
        const [huberSum] = yield* values(yield* Loss.huber(pred, target, { reduction: "sum" }))
        expect(Math.abs(huberSum - 4)).toBeLessThan(1e-12)
      })
    )
    ...
    it.effect("binaryCrossEntropy from probabilities and logits", () =>
      Effect.gen(function* () {
        ...
        expect(Math.abs(bce - expected)).toBeLessThan(1e-12)
        ...
        expect(Math.abs(fromLogits - (stable(2, 1) + stable(-1, 0)) / 2)).toBeLessThan(1e-12)
      })
    )
    ...
    it.effect("klDiv / hinge / cosineEmbeddingLoss", () =>
      Effect.gen(function* () {
        ...
        expect(Math.abs(cos - 0)).toBeLessThan(1e-9)
      })
    )
