...
pub fn matmul(a: &MetalTensor, b: &MetalTensor) -> crate::err::Res<MetalTensor> {
    if a.dtype != b.dtype {
        return Err(format!("matmul: dtype mismatch, got {:?} and {:?}", a.dtype, b.dtype));
    }
...
pub fn conv1d(x: &MetalTensor, w: &MetalTensor, stride: usize, padding: usize, dilation: usize, groups: usize) -> crate::err::Res<MetalTensor> {
    let xn = contig(x)?;
    let wn = contig(w)?;
    conv::conv1d(MetalDevice::get(), &xn, &wn, stride, padding, dilation, groups)
}

pub fn conv2d(x: &MetalTensor, w: &MetalTensor, stride: usize, padding: usize, dilation: usize, groups: usize) -> crate::err::Res<MetalTensor> {
    let xn = contig(x)?;
    let wn = contig(w)?;
    conv::conv2d(MetalDevice::get(), &xn, &wn, stride, padding, dilation, groups)
}

#[allow(clippy::too_many_arguments)]
pub fn conv_transpose1d(x: &MetalTensor, w: &MetalTensor, stride: usize, padding: usize, output_padding: usize, dilation: usize, groups: usize) -> crate::err::Res<MetalTensor> {
    let xn = contig(x)?;
    let wn = contig(w)?;
    conv::conv_transpose1d(MetalDevice::get(), &xn, &wn, stride, padding, output_padding, dilation, groups)
}

#[allow(clippy::too_many_arguments)]
pub fn conv_transpose2d(x: &MetalTensor, w: &MetalTensor, stride: usize, padding: usize, output_padding: usize, dilation: usize, groups: usize) -> crate::err::Res<MetalTensor> {
    let xn = contig(x)?;
    let wn = contig(w)?;
    conv::conv_transpose2d(MetalDevice::get(), &xn, &wn, stride, padding, output_padding, dilation, groups)
}

#[allow(clippy::too_many_arguments)]
pub fn conv2d_backward_w(x: &MetalTensor, g: &MetalTensor, kernel: [usize; 2], out_channels: usize, stride: usize, padding: usize, dilation: usize, groups: usize) -> crate::err::Res<MetalTensor> {
    let xn = contig(x)?;
    let gn = contig(g)?;
    conv::conv2d_backward_w(MetalDevice::get(), &xn, &gn, kernel, out_channels, stride, padding, dilation, groups)
}

pub fn gemm_bias(x: &MetalTensor, w: &MetalTensor, bias: &MetalTensor, batch: usize, m: usize, n: usize, k: usize) -> crate::err::Res<MetalTensor> {
    let xn = contig(x)?;
    let wn = contig(w)?;
    gemm::gemm(MetalDevice::get(), &xn, &wn, Some(bias), batch, m, n, k, m * k, 0)
}

pub fn linear(x: &MetalTensor, w: &MetalTensor, bias: &MetalTensor) -> crate::err::Res<MetalTensor> {
    require_f32(x)?;
    require_f32(w)?;
