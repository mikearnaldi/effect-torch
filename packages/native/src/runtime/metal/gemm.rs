...
fn gemm_source(bias: bool) -> String {
    let bias_decl = if bias {
        "    device const float* bias [[buffer(3)]],\n"
    } else {
        ""
    };
    let bias_add = if bias { " + bias[j]" } else { "" };
    format!(
        r#"
...
{bias_decl}    constant uint& M [[buffer(4)]],
    ...
    for (uint t = 0; t < K; t += {TILE}) {{
        ...
        As[tpitg.y][tpitg.x] = (i < M && ak < K) ? A[a_batch + (ulong)i * K + ak] : 0.0f;
        Bs[tpitg.y][tpitg.x] = (bk < K && j < N) ? B[b_batch + (ulong)bk * N + j] : 0.0f;
    ...
    if (i < M && j < N) {{
        D[d_batch + (ulong)i * N + j] = acc{bias_add};
    }}
}}
"#,
        TILE = TILE,
...
fn key_for(bias: bool) -> u64 {
    if bias { 0x6E11_B1A5 } else { 0x6E11_0000 }
}
...
pub fn gemm(
    ...
    let pipeline = dev.compile(key_for(has_bias), &gemm_source(has_bias), "et_gemm")?;
    let out = MetalTensor {
        buffer: dev.alloc(batch * m * n, DType::F32),
    ...
    dev.with_encoder(|e| {
        ...
        set_buffer(e, 0, &a.buffer, a.layout.offset() * 4);
        set_buffer(e, 1, &b.buffer, b.layout.offset() * 4);
        set_buffer(e, 2, &out.buffer, 0);
        if let Some(bias) = bias {
            set_buffer(e, 3, &bias.buffer, bias.layout.offset() * 4);
        }
        set_bytes(e, 4, &mu);
        set_bytes(e, 5, &nu);
        set_bytes(e, 6, &ku);
