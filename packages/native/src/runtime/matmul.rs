...
fn sgemm(a: &[f32], b: &[f32], out: &mut [f32], m: usize, n: usize, k: usize) {
    unsafe {
        matrixmultiply::sgemm(m, k, n, 1.0, a.as_ptr(), k as isize, 1, b.as_ptr(), n as isize, 1, 0.0, out.as_mut_ptr(), n as isize, 1);
    }
}

fn dgemm(a: &[f64], b: &[f64], out: &mut [f64], m: usize, n: usize, k: usize) {
    unsafe {
        matrixmultiply::dgemm(m, k, n, 1.0, a.as_ptr(), k as isize, 1, b.as_ptr(), n as isize, 1, 0.0, out.as_mut_ptr(), n as isize, 1);
    }
}

fn half_gemm_f32<T: Elem>(a: &[T], b: &[T], out: &mut [T], m: usize, n: usize, k: usize, to: impl Fn(T) -> f32, from: impl Fn(f32) -> T) {
    let af: Vec<f32> = a.iter().map(|&x| to(x)).collect();
    let bf: Vec<f32> = b.iter().map(|&x| to(x)).collect();
    let mut of = vec![0f32; m * n];
    sgemm(&af, &bf, &mut of, m, n, k);
    for (o, v) in out.iter_mut().zip(of) {
        *o = from(v);
    }
}
...
impl Tensor {
    pub fn matmul(&self, rhs: &Tensor) -> Tensor {
        ...
        let a_bc = self.view(self.layout.broadcast_to(&[batch_shape.as_slice(), self.shape()[a_rank - 2..].to_vec().as_slice()].concat()));
        let b_bc = rhs.view(rhs.layout.broadcast_to(&[batch_shape.as_slice(), rhs.shape()[b_rank - 2..].to_vec().as_slice()].concat()));
...
mod tests {
    ...
    fn matmul_2d() {
        ...
        let CpuBuffer::F32(v) = &c.buffer else { panic!() };
