...
impl Tensor {
    pub fn gather(&self, dim: usize, ids: &Tensor) -> Tensor {
        assert_eq!(ids.shape().len(), self.shape().len(), "gather: rank mismatch");
    ...
    pub fn scatter_add(&self, dim: usize, ids: &Tensor, src: &Tensor) -> Tensor {
        ...
        assert_eq!(ids.shape(), src.shape(), "scatter_add: indexes shape must match src shape");
        let flat = indices_vec(ids);
        let ids_shape = ids.shape().to_vec();
    ...
    pub fn cat(tensors: &[&Tensor], dim: usize) -> Tensor {
        ...
        macro_rules! go {
            ($variant:ident, $t:ty) => {{
                ...
                for t in tensors {
                    let tc = t.contiguous();
                    let CpuBuffer::$variant(data) = &tc.buffer else { unreachable!() };
