...
impl Xoroshiro128Plus {
    fn new(seed: u64) -> Self {
        ...
        Xoroshiro128Plus { s0: next(), s1: next() }
    }
...
impl Tensor {
    ...
    pub fn uniform(lo: f64, hi: f64, shape: &[usize], dtype: DType) -> Self {
        let n: usize = shape.iter().product();
        let data: Vec<f64> = with_rng(|rng| (0..n).map(|_| lo + (hi - lo) * rng.next_f64()).collect());
        Tensor::full(shape, 0.0, dtype).with_values(&data)
    }
...
mod tests {
    ...
    fn arange_eye() {
        ...
        let CpuBuffer::F32(v) = &a.buffer else { panic!() };
        assert_eq!(v.as_slice(), &[0.0, 2.0, 4.0]);
        let e = Tensor::eye(2, DType::F32);
        let CpuBuffer::F32(v) = &e.buffer else { panic!() };
    ...
    fn randn_deterministic() {
        ...
        let CpuBuffer::F32(x) = &a.buffer else { panic!() };
        let CpuBuffer::F32(y) = &b.buffer else { panic!() };
        assert_eq!(x, y);
    }
}
