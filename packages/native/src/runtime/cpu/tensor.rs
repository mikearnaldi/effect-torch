...
impl_elem!(bf16, BF16, DType::BF16, |x: bf16| x.to_f64(), bf16::from_f64);
...
impl Tensor {
    ...
    pub fn view(&self, layout: Layout) -> Self {
        ...
        Tensor { buffer: self.buffer.clone(), layout }
    }
...
fn contiguous_impl<T: Elem>(src: &[T], layout: &Layout) -> Tensor {
    ...
    Tensor::new(T::buffer_of(out), Layout::contiguous(layout.shape().to_vec()))
}

fn cast_impl<T: Elem>(src: &[T], shape: &[usize], dtype: DType) -> Tensor {
    macro_rules! to {
        ($t:ty) => {
            Tensor::from_vec(src.iter().map(|&x| <$t>::from_f64(x.to_f64())).collect::<Vec<$t>>(), shape.to_vec())
        };
    }
    match dtype {
        DType::F32 => to!(f32),
...
mod tests {
    ...
    fn contiguous_gathers_strided() {
        ...
        let CpuBuffer::F32(data) = &c.buffer else { panic!() };
        assert_eq!(data.as_slice(), &[0., 4., 8., 1., 5., 9., 2., 6., 10., 3., 7., 11.]);
    }
    ...
    fn cast_roundtrip_f16() {
        ...
        let CpuBuffer::F32(data) = &back.buffer else { panic!() };
    ...
    fn cast_int_float() {
        ...
        let CpuBuffer::F32(data) = &f.buffer else { panic!() };
        assert_eq!(data.as_slice(), &[1.0, 2.0, 255.0]);
        let b = f.cast(DType::U8);
        let CpuBuffer::U8(data) = &b.buffer else { panic!() };
