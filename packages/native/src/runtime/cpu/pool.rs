...
impl Slab {
    pub fn new(rows: usize, row_width: usize, dtype: DType) -> Self {
        ...
        Slab { data: RwLock::new(data), dtype, rows, row_width }
    }
    ...
    pub fn write_rows_u8(&self, rows: &[u32], src: &[u8]) {
        ...
        let w = self.row_width;
        let mut data = self.data.write().unwrap();
        let SlabData::U8(buf) = &mut *data else { panic!("write_rows_u8 on a non-u8 slab") };
        for (i, &r) in rows.iter().enumerate() {
            let dst = r as usize * w;
            buf[dst..dst + w].copy_from_slice(&src[i * w..(i + 1) * w]);
        }
...
mod tests {
    ...
    fn int8_roundtrip() {
        ...
        let back = dequantize_int8(&q.iter().map(|&v| v as f32).collect::<Vec<_>>(), &s, 2, 2, 2);
