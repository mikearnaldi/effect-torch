__device__ void layer_norm_f64(
    unsigned int op,
    const double *x,
    const double *weight,
    const double *other,
...
__device__ void rotary_f64(
    ...
    unsigned int position = (offsets ? offsets[leading / groups] : 0) + (index / width) % seq_len;
