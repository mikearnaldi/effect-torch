...
pub fn capture_end(live: &HashSet<usize>) -> CaptureResult {
    ...
    CAPTURE.with(|c| {
        let cap = c
            .borrow_mut()
            .take()
            .expect("capture_end without capture_begin");
        ...
        let intervals = cap
            ...
            .map(|e| {
                if escapes.contains(&e.clock) {
                    ...
                    Some((
                        e.clock,
                        cap.deaths.get(&e.clock).copied().unwrap_or(cap.clock),
                    ))
                }
            })
            .collect();
        CaptureResult {
            sizes,
            intervals,
            kinds: cap.events.iter().map(|e| e.kind).collect(),
            live_at_end: live.len(),
            checkpoints: cap.checkpoints,
            max_open: cap.max_open,
        }
    })
}
...
mod tests {
    ...
    fn best_fit_picks_the_smallest_region() {
        ...
        let captured = cr(
            vec![1024, 256, 200, 1000],
            vec![Some((1, 2)), Some((2, 3)), Some((4, 5)), Some((4, 6))],
        );
        let (entries, total) = plan_layout(&captured);
        let offsets = offsets(&entries);
        assert_eq!(offsets[0], Some(0));
    ...
    fn coalescing_merges_adjacent_regions() {
        ...
        let captured = cr(
            vec![256, 256, 500],
            vec![Some((1, 3)), Some((2, 3)), Some((4, 5))],
        );
        let (entries, total) = plan_layout(&captured);
