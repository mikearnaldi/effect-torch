...
impl Tensor {
    ...
    pub fn max(&self, dims: &[usize]) -> Tensor {
        match &self.buffer {
            ...
            CpuBuffer::F16(v) => reduce_impl(
                v,
                &self.layout,
                dims,
                f16::NEG_INFINITY,
                |a: f16, b: f16| if a >= b { a } else { b },
            ),
            CpuBuffer::BF16(v) => reduce_impl(
                v,
                &self.layout,
                dims,
                bf16::NEG_INFINITY,
            ...
            CpuBuffer::U8(v) => {
                reduce_impl(v, &self.layout, dims, u8::MIN, |a: u8, b: u8| a.max(b))
            }
            CpuBuffer::U32(v) => {
                reduce_impl(v, &self.layout, dims, u32::MIN, |a: u32, b: u32| a.max(b))
            }
            CpuBuffer::I64(v) => {
                reduce_impl(v, &self.layout, dims, i64::MIN, |a: i64, b: i64| a.max(b))
            }
        }
    }
    ...
    pub fn min(&self, dims: &[usize]) -> Tensor {
        match &self.buffer {
            CpuBuffer::F32(v) => {
                reduce_impl(v, &self.layout, dims, f32::INFINITY, |a: f32, b: f32| {
                    if a <= b {
                        a
                    } else {
                        b
            ...
            CpuBuffer::F64(v) => {
                reduce_impl(v, &self.layout, dims, f64::INFINITY, |a: f64, b: f64| {
                    if a <= b {
                        a
                    } else {
                        b
            ...
            CpuBuffer::F16(v) => {
                reduce_impl(v, &self.layout, dims, f16::INFINITY, |a: f16, b: f16| {
                    if a <= b {
                        a
                    } else {
                        b
            ...
            CpuBuffer::BF16(v) => {
                reduce_impl(v, &self.layout, dims, bf16::INFINITY, |a: bf16, b: bf16| {
                    if a <= b {
                        a
                    } else {
                        b
            ...
            CpuBuffer::U8(v) => {
                reduce_impl(v, &self.layout, dims, u8::MAX, |a: u8, b: u8| a.min(b))
            }
            CpuBuffer::U32(v) => {
                reduce_impl(v, &self.layout, dims, u32::MAX, |a: u32, b: u32| a.min(b))
            }
            CpuBuffer::I64(v) => {
                reduce_impl(v, &self.layout, dims, i64::MAX, |a: i64, b: i64| a.min(b))
            }
        }
    }
...
mod tests {
    ...
    fn argmax_cumsum() {
        ...
        let am = t.argmax(1);
        let CpuBuffer::U32(v) = &am.buffer else {
            panic!()
        };
