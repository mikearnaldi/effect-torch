...
pub fn conv1d(
    ...
    let xf = xc.cast(DType::F64);
    let wf = wc.cast(DType::F64);
    let super::tensor::CpuBuffer::F64(xd) = &xf.buffer else {
        unreachable!()
    };
    let super::tensor::CpuBuffer::F64(wd) = &wf.buffer else {
        unreachable!()
    };
    let lp = l + 2 * padding;
    for b in 0..n {
                ...
                for i in 0..l_out {
                    ...
                    for ci in 0..c_per {
                        ...
                        for kk in 0..k {
                            ...
                            if pos < lp {
                                acc +=
                                    xd[(b * c_in + ic) * lp + pos] * wd[(oc * c_per + ci) * k + kk];
                            }
                        }
                    }
...
pub fn conv2d(
    ...
    let xc = x.contiguous().cast(DType::F64);
    let wc = w.contiguous().cast(DType::F64);
    let super::tensor::CpuBuffer::F64(xd) = &xc.buffer else {
        unreachable!()
    };
    let super::tensor::CpuBuffer::F64(wgt) = &wc.buffer else {
        unreachable!()
    };
...
pub fn conv_transpose1d(
    ...
    assert_eq!(
        c_in,
        w.shape()[0],
        "conv_transpose1d: weight dim 0 must equal input channels"
    );
...
pub fn conv_transpose2d(
    ...
    assert_eq!(
        c_in,
        w.shape()[0],
        "conv_transpose2d: weight dim 0 must equal input channels"
    );
    let cin_per = c_in / groups;
    let c_out = c_out_per_g * groups;
                        ...
                        for co in 0..c_out_per_g {
                            ...
                            for ky in 0..kh {
                                ...
                                for kx in 0..kw {
                                    ...
                                    out[((b * c_out + oc) * oh + (py - padding)) * ow
                                        + (px - padding)] +=
...
pub fn conv2d_backward_w(
    ...
    let xc = x.contiguous().cast(DType::F64);
    let gc = g.contiguous().cast(DType::F64);
    let super::tensor::CpuBuffer::F64(xd) = &xc.buffer else {
        unreachable!()
    };
    let super::tensor::CpuBuffer::F64(gd) = &gc.buffer else {
        unreachable!()
    };
